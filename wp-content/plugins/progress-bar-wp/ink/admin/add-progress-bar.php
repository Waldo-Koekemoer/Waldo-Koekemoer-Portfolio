...
<div style=" overflow: hidden;padding: 10px;">
	<div class="wpsm_site_sidebar_widget_title">
		<h4><?php esc_html_e('Add Progressbar',progress_bar_wp_text_domain); ?></h4>
	</div>
			...
			{ 
				if($TotalCount!=-1)
				{
					foreach($data as $single_data)
					{
						...
						<input type="text" id="progress_title[]" name="progress_title[]" value="<?php echo esc_attr($progress_title); ?>" placeholder="Enter Title Here" class="wpsm_ac_label_text">
						<span class="ac_label"><?php _e('Set ProgressBar Percentage',progress_bar_wp_text_domain); ?></span> 
                        <div>
                        <input type="range" min="1" max="100" name="progress_size[]" value="<?php echo esc_attr($progress_size); ?>" data-rangeSlider>
                        <output></output>
                        </div>	
				...
				echo "<h2>No ProgressBar Found</h2>";
				}
			}
			else 
			{
